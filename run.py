...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print('Usage: python run.py "description of change"')
        sys.exit(1)
    ...
    print(f"\n{'='*55}")
    print(f"  Experiment {exp_id:03d}: {description}")
    print(f"{'='*55}")
    ...
    if best_so_far is None:
        ...
        status_hint = "keep"
        decision    = f"IMPROVED (+{auc_roc - best_so_far:.4f}) — KEEP this change"
    else:
        status_hint = "revert"
        decision    = f"NO IMPROVEMENT ({auc_roc - best_so_far:+.4f}) — REVERT pipeline.py"
